...
class AdminController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        $move = Storage::disk('s3')->put('fotos/nieuws/thumbnails/' . $filename, $image);
        Storage::disk('s3')->setVisibility('fotos/nieuws/thumbnails/'.$filename, 'public');

        if($move){
            return Response::json(['file'=> ['filelink' => ['http://svforum.s3.eu-central-1.amazonaws.com/fotos/nieuws/thumbnails/'. $filename]]]);
        }else{
